...
root = Tk()
root.withdraw()
folder_path = filedialog.askdirectory(title="选择包含 ion_positions_batch.csv 的文件夹")
if not folder_path:
...
for img_name, group in tqdm(df.groupby("Filename"), desc="Processing images", ncols=100):
    ...
    if n_missing > 0:
        ...
        avg_dist_center = np.mean(np.diff(x_sorted[center_indices])) if len(center_indices) > 1 else np.mean(np.diff(x_sorted))
    ...
    # ================== kink 离子标记 ==================
    is_kink = np.array([False if pred else abs(y) < axis_threshold for y, pred in zip(y_all, predicted_mask)])
    ...
    for j in range(len(real_indices)-1):
        ...
        if defect_type[idx_prev_real] == 'kink_defect' or defect_type[idx_next_real] == 'kink_defect':
            continue

        defect_type[idx_next_real] = 'normal_defect'

    # ==========================================================
    # 【新增】二次 quench 判定：连续缺陷 → quench_boundary_defect
    # ==========================================================
    ...
    if len(defect_indices) > 0:
        ...
        for block in blocks:
            if len(block) >= 4:
                ...
                for i in block:
                    if i == keep:
                        defect_type[i] = 'quench_boundary_defect'
                    else:
                        defect_type[i] = ''
